...
typedef struct kl_mesh {
  int  tris_i, tris_n; /* starting index and count */
} kl_mesh_t;

typedef struct kl_model {
  int bufs[6]; /* several vertex buffer objects */
  int tris;    /* element array buffer */
  int attribs; /* a vertex array object (or equivalent) */
  int mesh_n;
  kl_mesh_t mesh[];
} kl_model_t;
